...
public class Ex14_02 {
	...
	public static void main(String[] args) {
		...
		//참조타입(String) 1차원 배열 선엉
		String[] snake = {"구렁이","팔렁이","칠렁이","육렁이","오렁이"};
		String[] insect= {"메뚜기","개미","지렁이"};
		
		//참조타입의 배열의 각 요소의 초기값은 기본값이 null로 되어 있음
		//두배열 합치기 [null][null][null][null][null][null][null][null][null]
		String[] animal = new String[snake.length +insect.length+1]; //+1해줘야함
		
		//합치기
		for(int i=0; i<snake.length; i++) {animal[i]=snake[i];}
		for(int i=0; i<insect.length; i++) { animal[snake.length+i]=insect[i];} //+i 해줘야함
		...
		System.out.println(snake[0].hashCode()); //snske[0] 까지가 String객체
